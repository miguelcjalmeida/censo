...
class PacienteDAL{
	...
	public static function getPorId($id){
		...
		email,autista,id_escolaridade,id_instituicao,id_motivo_trabalho,renda,id_motivo_tratamento,especificacaoDeficiencia from paciente where id='$id' and desativado = 'n'"));
		$p->id=$id;
		$p->nome=$o->nome;
		$p->sexo=$o->sexo;
	...
	public static function cadastrar(PacienteTO $p){
	   ...
	   $query = ("insert into paciente(id,nome,sexo,nascimento,responsavel,email,autista,renda,id_escolaridade,id_instituicao,id_motivo_trabalho,id_motivo_tratamento,id_endereco,especificacaoDeficiencia,desativado)
		values($id,'{$p->nome}','{$p->sexo}','{$p->nascimento}','{$p->responsavel}','{$p->email}','{$autista}',$renda,$id_escolaridade,$id_instituicao,$id_motivo_trabalho,$id_motivo_tratamento,'$id_endereco','$p->especificacaoDeficiencia','n')");
